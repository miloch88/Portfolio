class Game {
  constructor(start) {
    ...
    this.spanLost = document.querySelector('.score span.loss');
  ...
  render(colors = ['gray', 'gray', 'gray'], money = this.wallet.getWalletValue(), result = '', stats = [0, 0, 0], bid = 0, wonMoney = 0) {
    ...
    this.spanLost.textContent = stats[2];

    this.inputBid.value = '';
  }

  startGame() {
    if (this.inputBid.value < 1) return alert('Kwot, którą chcesz grać jest za mała');
